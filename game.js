/* منطق لعبة مخرِّب الرياض، بدون شبكة ولا رسم.
   السيرفر (server.js) يستخدمه مع الساعة الحقيقية، والمحاكاة (sim.js) بساعة افتراضية أسرع.
   كل ما يتعلق بالحالة والقواعد والبوتات هنا. كل ما يتعلق بالاتصال والملفات في server.js. */
"use strict";
const crypto = require("crypto");
const S = require("./public/shared.js");
const C = S.C;

/* opts.now  : دالة ترجّع الوقت بالمللي ثانية (Date.now أو ساعة افتراضية)
   opts.onKick: تُنادى لما يُطرد لاعب متصل، عشان السيرفر يبلّغ عميله */
function createGame(opts = {}) {
  const now = opts.now || Date.now;
  const onKick = opts.onKick || (() => {});
  ...
  function startGame() {
    if (G.players.length < C.MIN_PLAYERS) return false;
    ...
    G.players.forEach((p) => {
      ...
      p.killReady = now() + C.FIRST_KILL;
      p.sabReady = now() + C.SAB_FIRST;
    });
    G.sabUntil = 0;
    G.bodies = [];
    G.doneTasks = 0;
  ...
  /* الرجوع للوبي بعد نهاية الجولة، بنفس اللاعبين */
  function again() {
  ...
  function doKill(killer, victim) {
    victim.alive = false;
    killer.killReady = now() + C.KILL_CD;
    killer.x = victim.x;
    killer.y = victim.y;
  ...
  function handle(me, m) {
    switch (m.t) {
      ...
      case "bots":
        ...
        break;
      case "again":
        if (me.id === G.hostId) again();
        break;
      case "move": {
        if (G.phase !== "play") break;
        const nowMs = now();
        const dt = Math.min(0.4, (nowMs - me.lastMove) / 1000);
        me.lastMove = nowMs;
        const x = +m.x, y = +m.y;
      ...
      case "task": {
        if (G.phase !== "play" || !me.alive || me.imp) break;
        if (blackout(now())) break; // ما فيه كهرب، ما فيه مهام
      ...
      case "vote": {
        ...
        if (alive.every((p) => p.vote !== null)) G.meetEnd = Math.min(G.meetEnd, now() + 3000);
        break;
      }
    }
  }
  ...
  /* ما يوصل للاعب معيّن: السيرفر يفلتر حسب رؤيته، والمتصفح يرسم بس */
  function snapshotFor(me, nowMs) {
    ...
    return {
      ...
      dt: G.doneTasks,
      tt: G.totalTasks,
      left: Math.max(0, Math.ceil((G.meetEnd - nowMs) / 1000)),
      sab: blackout(nowMs) ? Math.ceil((G.sabUntil - nowMs) / 1000) : 0, // ثواني الظلمة الباقية
      res: G.result,
      win: G.winner,
      ...
      me: {
        ...
        cd: Math.max(0, Math.ceil((me.killReady - nowMs) / 1000)),
        scd: me.imp ? Math.max(0, Math.ceil((me.sabReady - nowMs) / 1000)) : 0,
      },
      ps: G.players
        .filter((p) => p.id !== me.id)
        .filter((p) => showAll || (p.alive && canSee(me, p.x, p.y, nowMs)))
      ...
      bd: G.bodies
        .filter((b) => showAll || canSee(me, b.x, b.y, nowMs))
  ...
  return {
    ...
    tick,
    snapshotFor,
    findByToken: (token) => G.players.find((x) => x.token === token),
  };
}
